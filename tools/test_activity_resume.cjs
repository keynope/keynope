...
const context = vm.createContext({Date:{now:()=>now},clearInterval:noop,crypto:require('node:crypto').webcrypto,engagementBase64URL:bytes=>Buffer.from(bytes).toString('base64url'),
  window:{},archiveEngagementResult:noop,engagementPresentationActive:()=>true,keynopeRunningActivity:null,renderActiveActivityStatus:noop,
...
for(const name of ['engagementIsRunning','engagementHasCompleted','onboardingSessionDefinition','engagementSessionFor','currentEngagementDefinition','activitySubmittedItems','closeEngagementRuntime','openEngagementRuntime','resetEngagementRuntime']) {
...
assert.equal(r.deadlineMs,0);assert.equal(r.completedReview,true);
...
assert.equal(context.keynopeEngagementRuntime.pausedRemainingMs,0);
...
r=context.keynopeEngagementRuntime;
assert.equal(r.phase,0);assert.equal(r.ideas.length,0);assert.equal(r.questionIndex,0);assert.equal(r.game,undefined);
context.closeEngagementRuntime();context.keynopeEngagementSessions.clear();context.openEngagementRuntime(false);
assert.equal(context.keynopeEngagementRuntime.phase,0);
