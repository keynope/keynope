...
func TestWebExportUsesPresentationCanvasRenderer(t *testing.T) {
	...
	for _, marker := range []string{
		...
		"state.version === presenterVersion && !slideRefreshed",
		"if (slideRefreshed && keynopeAppSurface && editorState) renderEditorTopbar();",
		"canvasAuthoredTextMetric(element,'modern-size',modern.size)",
		"canvasAuthoredParagraphMetrics(e,text)",
		"if(Object.hasOwn(values,'textSizeDelta')||Object.hasOwn(values,'textWidthDelta'))renderEditorTopbar();",
		"sizeControl.className='keynope-text-metric-control'",
		"widthControl.className='keynope-text-metric-control'",
		...
		"dirty === lastPublishedEditorDirty && !window.KEYNOPE_WEB_EDITOR",
		...
		"Add emoji",
		"limit:'72', size:'10'",
		"query:'render=truetype&ttf-size=512'",
		...
		"const undoButton = canvasTool('⟲'",
		"const redoButton = canvasTool('⟳'",
		...
		"editorState.version !== stateVersion",
		"editorMutationPreviewSequence++;",
		"moveCanvasMembers(canvasMoveSnapshot(indices),dx,dy)",
		...
		"activeCanvasDrag.indices.includes(Number(hit.dataset.element))",
